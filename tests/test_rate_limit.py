...
def test_failed_logins_lock_and_success_clears(tmp_path: Path):
    ...
    allowed, remaining = limiter.record_failure("198.51.100.10", now=102)
    assert allowed is False
    assert remaining == 0
    allowed, retry_after = limiter.check("198.51.100.10", now=120)
    assert allowed is False
...
def test_window_expires_and_state_is_durable(tmp_path: Path):
    ...
    assert second.record_failure("203.0.113.7", now=102) == (False, 0)
    assert second.check("203.0.113.7", now=102) == (False, 28)
