...
class OwnershipStore:
    """Durable user/project ownership primitives.

    This module is intentionally small so ownership can be introduced without
    rewriting the existing application schema. Legacy projects are assigned to
    the bootstrap administrator (user id 1) during migration.
    """
    ...
    def initialize(self) -> None:
        """Create ownership tables and migrate legacy projects safely."""
        with self._connect() as connection:
            ...
            connection.execute(
                """
                INSERT OR IGNORE INTO users(id, username, is_active, created_at)
                VALUES (1, 'admin', 1, datetime('now'))
                """
            )
            ...
            connection.execute(
                "UPDATE projects SET owner_id = 1 WHERE owner_id IS NULL OR owner_id <= 0"
            )
            connection.execute(
    ...
    def create_user(self, username: str) -> int:
        ...
        with self._connect() as connection:
            cursor = connection.execute(
                "INSERT INTO users(username, is_active, created_at) VALUES (?, 1, datetime('now'))",
                (username,),
            )
    ...
    def list_project_ids(self, user_id: int) -> list[int]:
        with self._connect() as connection:
            rows = connection.execute(
                "SELECT id FROM projects WHERE owner_id = ? ORDER BY id",
                (user_id,),
            ).fetchall()
